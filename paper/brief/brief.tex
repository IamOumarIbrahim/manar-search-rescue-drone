...
\title{MANAR: Supervised Autonomy, Deterministic Control, and Greedy Route Optimization for Aerial Search-and-Rescue\\
...
\begin{abstract}
Aerial Search-and-Rescue (SAR) requires reliable area coverage, deterministic execution, and strict operational safety. MANAR (Multi-sensor Autonomous Navigation and Aerial Rescue) is a supervised-autonomy UAV framework balancing human oversight against automated execution. This brief outlines MANAR's core system architecture, drawing an explicit distinction across three maturity tiers: the currently functional C++ control core, the selected state-ownership interface architecture, and planned perception pipelines. The implemented prototype features double-precision Haversine geodetics, an $O(n^2)$ greedy nearest-neighbor route optimizer with operator override privilege, sequential lawnmower sweeps, and deterministic battery failsafes. We outline the multi-modal payload suite, passive RF attention logic, and adaptive telemetry power model governing real-time operations.
\end{abstract}
...
\midrule
\textbf{Perception} & PyTorch Vision / Radar / Audio & \textcolor{tagplan}{\textbf{\ding{109} Planned}} \\
...
\begin{tikzpicture}[node distance=0.8cm, auto, >=stealth', font=\sffamily\footnotesize]
    ...
    \node [subblock, below right=0.8cm and -0.2cm of core] (perception) {\textbf{Perception / ML}\\{\scriptsize Python/PyTorch \textit{(Planned)}}};
...
\begin{tikzpicture}[font=\sffamily\scriptsize]
    \draw[thick, fill=blue!5, rounded corners=2pt] (0,0) rectangle (2.8,2.3);
    \node[anchor=south west, font=\tiny\bfseries] at (0,2.3) {Lawnmower Grid ($W \times H$)};
    \draw[->, thick, red!80!black] (0.2,0.2) -- (2.6,0.2) -- (2.6,0.75) -- (0.2,0.75) -- (0.2,1.35) -- (2.6,1.35) -- (2.6,1.95) -- (0.2,1.95);
    \draw[<->, dashed, gray] (2.9,0.2) -- (2.9,0.75) node[midway, right, font=\tiny] {$\Delta y$};
    
    \begin{scope}[xshift=4.0cm, yshift=0.1cm]
        \node[draw, circle, fill=green!30, inner sep=1.5pt] (p0) at (0,0.2) {$p_0$};
        \node[draw, circle, fill=blue!20, inner sep=1.5pt] (L1) at (0.7,1.8) {$L_1$};
        \node[draw, circle, fill=blue!20, inner sep=1.5pt] (L2) at (2.3,1.4) {$L_2$};
...
\section{Multisensor Payload \& Planned Perception [Tier 3]}
...
\begin{tikzpicture}[
    ...
    role/.style={draw=blue!60!black, fill=blue!8, rounded corners=2pt, text width=3.8cm, minimum height=0.44cm, align=left, inner sep=3pt, line width=0.5pt},
    modality/.style={draw=green!60!black, fill=green!8, rounded corners=2pt, text width=3.7cm, minimum height=0.44cm, align=left, inner sep=3pt, line width=0.5pt},
    guide/.style={draw=orange!80!black, fill=orange!8, rounded corners=2pt, text width=3.7cm, minimum height=0.44cm, align=left, inner sep=3pt, line width=0.5pt},
    link/.style={->, line width=0.7pt, gray!70!black}
]
    \node[role] (r1) at (0, 3.3) {\textbf{Human visual detection}};
    \node[role] (r2) at (0, 2.75) {\textbf{Heat detection}};
    \node[role] (r3) at (0, 2.2) {\textbf{Night confirmation}};
    \node[role] (r4) at (0, 1.65) {\textbf{Motion / breathing detection}};
    \node[role] (r5) at (0, 1.1) {\textbf{Voice response}};
    \node[role] (r6) at (0, 0.55) {\textbf{Electronic-device detection}};
    \node[role] (r7) at (0, 0.0) {\textbf{Rescuee guidance}};

    \node[modality] (m1) at (4.8, 3.3) {Day RGB Camera};
    \node[modality] (m2) at (4.8, 2.75) {Thermal LWIR Sensor};
    \node[modality] (m3) at (4.8, 2.2) {Low-Light / IR Camera};
    \node[modality] (m4) at (4.8, 1.65) {24 GHz FMCW Radar};
    \node[modality] (m5) at (4.8, 1.1) {Speaker + Microphone Array};
    \node[modality] (m6) at (4.8, 0.55) {Passive RF SDR ($\le 6\text{ GHz}$)};
    \node[guide]    (m7) at (4.8, 0.0) {Beacon / Strobe / Light / Smoke};
...
\begin{itemize}
    ...
    \item \textbf{Acoustic Call-and-Listen Cycle:} $35.4\text{ s}$ sequence ($8.4\text{ s}$ SOS buzzer, $12.0\text{ s}$ Arabic/English voice prompt, $15.0\text{ s}$ silent listening with motor-RPM harmonic cancellation).
    \item \textbf{Passive RF Two-Level Attention ($\le 6\text{ GHz}$):} Level 1 CFAR threshold ($N_{\mathrm{CFAR+}} \ge 3$) triggers Level 2 temporal trend tracking ($\Delta S_k$) in \texttt{Inspect} mode (HOT / STABLE / COLD).
    \item \textbf{FMCW Radar Ranging:} $24\text{ GHz}$ radar ($B \approx 250\text{ MHz}$) provides $\Delta R \approx c/(2B) \approx 0.60\text{ m}$ range resolution for breathing micro-motion.
    \item \textbf{Mamba SSM Alert Gate:} Evaluates synchronized $10\text{ Hz}$ feature vectors $\mathbf{x}_t \in \mathbb{R}^D$ over $20\text{--}50$ frames ($2\text{--}5\text{ s}$). Mamba gates alert escalation; human operator assigns final rescue status.
    \item \textbf{Rescue Guidance:} $360^\circ$ amber beacon, $2\text{ Hz}$ white strobe, spotlight, smoke marker, and 3 fixed heliograph mirrors ($40^\circ, 60^\circ, 75^\circ$).
\end{itemize}

\section{Adaptive Telemetry Power Model}
Incremental RF transmission power $\Delta P_i$ across frequencies $f_i$ is formulated as:
\begin{equation}
...
\textbf{Functional Verification:} Testing confirmed deterministic C++ control flow, search plan locking, start rejection on empty plans, and Return-to-Launch (RTL) preemption under Windows 11 / GCC.

\textbf{System Roadmap:}
\begin{enumerate}
    \item \emph{Interface Layer:} Complete React/TypeScript WebSocket GCS.
    \item \emph{Perception Training:} Train CNN/Mamba models on SAR datasets.
