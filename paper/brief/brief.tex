...
\begin{itemize}
    \item \textbf{Visual Candidate Detection (YOLO11n):} Evaluated across 26 SAR video datasets. Operates at $29.2\text{ ms}$ average latency ($\approx 34\text{ FPS}$ on CPU, $1.73\times$ faster than $50.5\text{ ms}$ D-FINE-N baseline) with $100\%$ clean negative rejection ($0\text{ FP}$) and superior concealed-target sensitivity.
...
\textbf{Functional Verification:} Testing confirmed deterministic C++ control flow, search plan locking, start rejection on empty plans, Return-to-Launch (RTL) preemption, and real-time ONNX Runtime visual candidate detection under Windows 11 / GCC.
