\section{Analytical Telemetry Policy [Planned]}

To study bandwidth--energy tradeoffs before radio integration, MANAR proposes an adaptive telemetry policy and a first-order airtime model. Neither the rate controller nor its energy savings are implemented or measured in the present artifact. The analysis is retained as a design hypothesis for future hardware trials.

\subsection{Analytical Radio Telemetry Power Model}
Radio frequency (RF) telemetry power consumption is approximated as scaling linearly with payload airtime. Let $P_{\mathrm{TX}}$ denote transmitter active power, $P_{\mathrm{RX}}$ denote receiver baseline power, and $R$ denote link channel data rate in bits per second (bps).
...
For an illustrative link with $P_{\mathrm{TX}} = 4.80\text{ W}$, $P_{\mathrm{RX}} = 0.36\text{ W}$, and channel rate $R = 64\,000\text{ bit/s}$, the slope coefficient evaluates to $\beta \approx 5.55 \times 10^{-4}\text{ W}/(\text{byte}\cdot\text{Hz})$. The model excludes headers, acknowledgements, contention, retransmission, state-transition energy, coding, idle policy, and rate adaptation; it must not be read as a radio power measurement.
...
Table~\ref{tab:telemetry_rates} specifies a candidate dual-regime rate policy for later implementation and measurement.
...
Under the proposed policy, routine navigation streams update at $2\text{ Hz}$, slow-varying battery state at $0.2\text{ Hz}$, and discrete states on change. A candidate event would switch relevant streams to $5$--$10\text{ Hz}$. Future work must implement the scheduler, measure packet-level traffic and energy, and test whether the increased update rate improves operator localization enough to justify its cost.
